...
try:
    connection = connector.connect(host="localhost", user="root", port="3306", password="tP_kc8mn",
    ...
    try:
        with closing(connection.cursor()) as cursor:
            query1 = ("INSERT INTO products_list "
                      "(product_name, sponsored, product_price, product_mrp, product_offer, product_deal, product_image, product_link, hash_id)"
                      "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)")

            cursor.execute(query1, data1)
            connection.commit()

            query2 = ("INSERT INTO categorys_list (category_name, category_link, hash_id, status) VALUES(%s, %s, %s, %s)")
...
def get_next_page(url):
    ...
    if response.status_code == 200:
        ...
        for pl in product_list:
            ...
            product_price = pl.xpath(".//div[@class='Nx9bqj']/text()").get('')
            product_mrp = "".join((pl.xpath(".//div[@class='yRaY8j']/text()")).getall())
            ...
            concatenated_data = ''.join(filter(None, [product_name, sponsored, product_price, product_mrp, product_offer,
                                                      product_deal, product_image]))
            ...
            data1 = (product_name, sponsored, product_price, product_mrp, product_offer,
                     product_deal, product_image, product_link, hash_id)
