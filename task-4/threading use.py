...
headers1 = {
    ...
    'Origin': 'https://www.simplotfoods.com',
    'Referer': 'https://www.simplotfoods.com/',
...
collection_category, collection_product_link = get_mongo_connection()
    ...
    while True:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if page < product_link_data['results'][0]['nbPages'] - 1:
                    page += 1
                else:
                    break
            else:
...
def process_subcategory(subcategory_link):
    ...
    for p_link in product_links:
        ...
        try:
            ...
            print("Error inserting to collection:", e)
            continue

    try:
        collection_category.update_one({"subcategory": sub_category_name}, {"$set": {"status": "done"}})
        print(f"Subcategory {sub_category_name} processed.")
    except Exception as e:
...
def main():
    ...
    for subcategory_link in subcategory_links:
        thread = threading.Thread(target=process_subcategory, args=(subcategory_link,))
        thread.start()
